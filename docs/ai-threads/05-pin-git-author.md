...
group:
  ...
  order: 5
title: 'Tip #1 — Pin the git author per repo'
order: 2
toc: content
description: How a Claude Code PreToolUse hook plus a durable rule keep the wrong git author from landing on a commit in a multi-agent multi-repo setup.
keywords:
  [
    ai,
    parallel,
...
# Pin the git author per repo

> Tip #1 in the [Work in parallel with AI](./05-work-in-parallel.md) series. Start with the [Introduction](./05-work-in-parallel.md) if you haven't already.
